...
const UserVerificationScreen = ({ navigation }) => {
  ...
  // Account type selection: 3 = Student, 2 = Mentor, 7 = VodjaTima
  ...
  const canAccess = user?.IdUserType === 4 || user?.IdUserType === 1; // koordinator nauka ili admin
  ...
  const fetchPending = async () => {
    ...
    try {
      ...
      const res = await apiClient.get("/user-verification", {
  ...
  const approve = async (id, newUserTypeId) => {
    try {
      setSubmitting(true);
      ...
      showAlert("OK", res.data?.message || "Verifikovano.");
      setData((prev) => prev.filter((u) => u.IdUser !== id));
    } catch (e) {
      showAlert("Greška", e.response?.data?.message || "Neuspješna verifikacija.");
    } finally {
      setSubmitting(false);
  ...
  const handlePick = (typeId) => {
    ...
    approve(selectedUser.IdUser, typeId);
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Upravljanje nalozima</Text>
      ...
      <View style={styles.accountTypeSelector}>
        ...
        <TouchableOpacity
          style={[styles.typeButton, selectedAccountType === 2 && styles.typeButtonActive]}
          onPress={() => handleAccountTypeChange(2)}
        >
          <Text style={[styles.typeButtonText, selectedAccountType === 2 && styles.typeButtonTextActive]}>
            Mentor
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>
            {searchQuery ? "Nema rezultata pretrage." : `Nema ${getAccountTypeName().toLowerCase()}a.`}
          </Text>
        </View>
      ) : (
        <FlatList
      ...
      {/* Prilagođeni modal za izbor uloge */}
      <Modal
        transparent
      ...
      >
        ...
        >
          {/* Stop propagation da tap unutar kartice ne zatvara modal */}
          <Pressable style={styles.modalCard}>
            <Text style={styles.modalTitle}>
              {selectedAccountType === 3 ? "Verifikacija" : "Promjena tipa naloga"}
            </Text>
            <Text style={styles.modalSubtitle}>
              {selectedAccountType === 3
                ? `Odaberi tip za ${selectedUser?.Name} ${selectedUser?.Lastname}`
                : `Vrati ${selectedUser?.Name} ${selectedUser?.Lastname} u studenta?`}
            </Text>

            {selectedAccountType === 3 ? (
              // Show promotion options for students
              <View style={styles.roleButtonsRow}>
                <TouchableOpacity
                  ...
                  disabled={submitting}
                >
                  <Text style={styles.roleButtonLabel}>MENTOR</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  disabled={submitting}
                >
                  <Text style={styles.roleButtonLabel}>VOĐA TIMA</Text>
                </TouchableOpacity>
              </View>
            ) : (
              // Show demotion option for mentors/team leaders
              <TouchableOpacity
                activeOpacity={0.85}
                ...
                disabled={submitting}
              >
                <Text style={styles.demoteButtonLabel}>DEGRADIRAJ U STUDENTA</Text>
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  modalCard: {
    ...
    paddingBottom: 40,
    paddingLeft: 20,
    // suptilna sjena kao i kartice u app-u
    shadowColor: "#000",
